...
CREATE TABLE `gofun`.`operations` (
  `operation_id` int(11) unsigned NOT NULL AUTO_INCREMENT,  
  `elevator_id` int(11) unsigned NOT NULL,
  `load_size` int(11) unsigned NOT NULL DEFAULT '1',
...
CREATE TABLE `gofun`.`configurations` (
...
INSERT INTO `gofun`.`elevators` (`name`,  `max_size`,  `status`)
VALUES
  ('Elevator1', 5, 1),
  ('ElevatorOFF', 8, 0);

INSERT INTO `gofun`.`configurations` ( `conf_key`,  `conf_value`,  `description`)
VALUES
  ...
  select * from `gofun`.`configurations`;
